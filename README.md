## Cisco DevNet Learning Labs: Python Fundamentals

These self-paced interactive tutorials provide instructions for developers to use Python.
...
These labs teach how to use the basics of Python environment, including the data types, variables, conditionals, and functions.
...
## Build offline static files

1. Make sure you have Python installed locally.
   ```
   $ python -V
   ```
1. Make sure you have installed `mkdocs`, a static site generator that will convert the Markdown to HTML files.
   ```
   $ pip install mkdocs
   ```
1. In the root of the repository, run:
   ...
   $ make offline
   ```
1. To provide the static files, copy the resulting `site` directory to a location where others can view the files.

> Note: You can run `mkdocs serve` to view live updates with the URL: `http://127.0.0.1:8000`. When you are done viewing, go to the Terminal and type `Ctrl+C` to stop running the `mkdocs serve` command.
